...
void MainWindow::heapify(QVector<long long>& arr, long long n, long long i, long long & count) {
    ...
    //ищем больший эл-т среди i, правого и левого
    ...
    //меняем местами и продолжаем, если i не больший
    if (largest != i) {
...
long long MainWindow::heapSort(QVector<long long> & arr, QString & text) {
    long long count = 0; //кол-во итераций
    long long step = 1;
...
long long MainWindow::quickSort(QVector<long long> & arr, long long low, long long high, QString & text)
{
    ...
    if (low == 0 && high == arr.size() - 1)
        step = 1;
    long long mid;
    long long f=low, l=high; //1-й и последний эл-ты
    mid=arr[(f+l) / 2]; //центральный эл-т
    while (f<l) {
        while (arr[f]<mid) f++;
        while (arr[l]>mid) l--;
        if (f<l)
...
long long MainWindow::merge(QVector<long long>& arr, long long l, long long m, long long r) {
    long long n1 = m - l + 1; //левый
    long long n2 = r - m; //правый
    ...
    //поддерживаем текущие индексы массива и подмассивов
    long long i = 0, j = 0, k = l;

    long long count = 0;

    //пока не достигнем конца подмассивов, выбираем больший эл-т
    //и ставим его в правильное положение
    while (i < n1 && j < n2) {
        ++count;
        if (L[i] <= R[j]) {
            arr[k] = L[i];
    ...
    //когда закончились эл-ты в подмассивах, помещаем оставшиеся на правильные места
    while (i < n1) {
        arr[k] = L[i];
        i++;
        k++;
...
long long MainWindow::binSearch(QVector<long long> & a, long long find) {
    ...
    while (r > l) {
        long long m = (l + r) / 2; //середина

        if (a[m] < find) { //средний эл-т меньше того числа, что ищем
            l = m + 1; //левый = средний + 1
        } else if (a[m] > find) {
            r = m - 1;
        } else {
            return m; //вернуть найденное
        }
    }
