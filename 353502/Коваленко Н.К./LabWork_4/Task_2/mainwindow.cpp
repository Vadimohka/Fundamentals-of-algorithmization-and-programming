...
void MainWindow::divide(QVector<long long>& A, QVector<long long>& divideSize, int & end) {
    auto size = divideSize.back(); //возвращает последний эл-т массива
    divideSize.pop_back(); //удалить последний эл-т
    auto start = end - size; //последний номер эл-та массива arr - кол-во эл-тов в исходном массиве
    auto min = A[start];
    auto max = A[start];
...
void MainWindow::interpolationSort(QVector<long long>& arr, QString & text) {
    ...
    int end = arr.size(); //кол-во элементов в массиве arr
    divideSize.push_back(end); //добавляем число: кол-во элементов в массиве arr
