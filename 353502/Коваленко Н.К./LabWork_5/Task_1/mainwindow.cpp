...
void MainWindow::Add_text_to_Widget(){
    ...
    ui -> Word_write_textedit -> append(text);*/
}
...
void MainWindow::Random_word_list(){
    ...
    {
        if(current_language == &bel_symbols) {
            int index = rand() % bel_symbols.length();
            QString nextChar = bel_symbols.at(index);
            randomString.append(nextChar);
        }
        if(current_language == &german_symbols) {
            int index = rand() % german_symbols.length();
            QString nextChar = german_symbols.at(index);
            randomString.append(nextChar);
        }
        if(current_language == &french_symbols) {
            int index = rand() % french_symbols.length();
            QString nextChar = french_symbols.at(index);
            randomString.append(nextChar);
        }
        if(current_language == &arabic_symbols) {
            int index = rand() % arabic_symbols.length();
            QString nextChar = arabic_symbols.at(index);
            randomString.append(nextChar);
        }
        if(current_language == &chinese_symbols) {
            int index = rand() % chinese_symbols.length();
            QString nextChar = chinese_symbols.at(index);
            randomString.append(nextChar);
        }
        if(current_language == &hebrew_symbols) {
            int index = rand() % hebrew_symbols.length();
            QString nextChar = hebrew_symbols.at(index);
            randomString.append(nextChar);
        }
    }
    ui -> Word_write_textedit -> setText(randomString);
}
...
void MainWindow::Last_letter_in_text_clear_color(){ //удаляет неправильно введенныйэ л-т (крю подсветку)
