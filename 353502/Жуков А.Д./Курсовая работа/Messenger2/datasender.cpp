...
void DataSender::sendFile(const QString &fileName, const QString &from, const QString &to, const QString &isGroup, const QString &content)
{
    sendToServer((fileName + "" + from + "" + to + "" + isGroup + "" + content).toUtf8(), "sendMessage_file");
}
...
void DataSender::sendToServer(const QByteArray &data, const QString &metadata) {
    ...
    package["data"] = QString::fromUtf8(data.toBase64());
...
void DataSender::readyReadSlot()
{
    ...
    if (metadata == "file") {
        QString outputFilePath = "path/to/save/file.txt";  // Укажите путь, куда сохранять файл
        receiveFileFromServer(JData, outputFilePath);
    } else {
        data = QByteArray::fromBase64((jsonObj["data"].toString()).toUtf8());
    }
