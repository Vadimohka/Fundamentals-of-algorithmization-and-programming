...
void Server::slotReadyRead()
{
    ...
    QString data = QByteArray::fromBase64((rootObject["data"].toString()).toUtf8());

    qDebug() << data << metadata;
    ...
    if (metadata == "newConnection") {
        ...
        sendOut(data, "text");
    } else if (metadata == "sendMessage_file") {
        sendOut(data, "file");
...
void Server::sendOut(const QString &data, const QString &type)
{
    ...
    if (type == "text") {
        ...
        databaseManager->addMessage(textMessage.first(), textMessage.at(1), textMessage.at(2), textMessage.at(3), "file", textMessage.last());
        newMessages.append(databaseManager->loadMessages(textMessage.at(1), textMessage.at(2)));
    }

    newMessages.last().chop(5);
    sendToClient(socket, newMessages.last().toUtf8(), "loadMessage_" + type);
    if (!textMessage.at(3).toShort()) {
        QString sendToChatId = databaseManager->getUID(textMessage.at(2));
        if (socketsStorage.contains(sendToChatId)) {
            ////////////////
            qDebug() << "TRYING TO SEND TO " << textMessage.at(1) << sendToChatId;
            ////////////////
