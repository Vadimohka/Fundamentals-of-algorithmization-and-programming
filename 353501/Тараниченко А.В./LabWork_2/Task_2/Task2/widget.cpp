...
widget::widget(QWidget *parent) : QWidget(parent)
{
    ...
    matText = new QLabel("", this);
    physText = new QLabel("", this);
    rusText = new QLabel("", this);
    atestatText = new QLabel("", this);
...
void widget::studTable(){
    ...
    QTableWidgetItem *average = new QTableWidgetItem(QString::number(stud[i].SumPoints()));
...
void widget::sort(int* a, int* b, int left, int right) {
    ...
    while (l < r) {
        while (a[l] > pos) { // Изменение: замена условия на a[l] > pos
            l++;
        }
        while (a[r] < pos) { // Изменение: замена условия на a[r] < pos
            r--;
        }
