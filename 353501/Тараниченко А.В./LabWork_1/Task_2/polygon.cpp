#include "polygon.h"
polygon::polygon() {}

polygon::~polygon()
{

}

void polygon::initialization()
{   now_area = (3 * sqrt(3) * pow(w, 2)) / 2; // Формула для площади шестиугольника
    now_perimeter = 6 * w; // Формула для периметра шестиугольника
    now_area = area;
    now_perimeter = perimeter;
}

void polygon::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    painter->setPen(hasFocus() ? QPen(QColor(255, 166, 21), 3) : QPen(QColor(95, 0, 45), 3));
    painter->drawPoint(0, 0);
    painter->setPen(QPen(QColor(R, G, B), PenSize));

    // Рисование шестиугольника
    QVector<QPointF> hexagonPoints;
    double angleRad = 2 * M_PI / 6; // Угол между сторонами
    double radius = w / 2; // Радиус описанной окружности
    for (int i = 0; i < 6; ++i) {
        double x = radius * cos(angleRad * i);
        double y = radius * sin(angleRad * i);
        hexagonPoints.append(QPointF(x, y));
    }
    painter->drawPolygon(hexagonPoints);

    QThread::msleep(10);

    double delta = ang - now_ang;
    now_ang += delta / 100;

    double deltaSize = size - now_size;
    now_size += deltaSize / 100;

    now_area = area * now_size * now_size;
    now_perimeter = perimeter * now_size;

    this->setRotation(now_ang);
    this->setScale(now_size);

    if (this->hasFocus())
        Output();

    Q_UNUSED(option);
    Q_UNUSED(widget);
}
