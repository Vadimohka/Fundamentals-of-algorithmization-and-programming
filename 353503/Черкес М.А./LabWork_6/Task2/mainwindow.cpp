...
void MainWindow::saveTextToFile(const QString& text) {
  QFile file(
      "C:\\Users\\Pojarnik\\Home\\MyQtLab\\Lab6\\untitled\\CherkesLab6.txt");
...
void MainWindow::task2() {
  ...
  textEdit->setMinimumSize(QSize(300, 300));
  layout->addWidget(textEdit);
  QFile file(R"(C:\Users\Pojarnik\Home\MyQtLab\Lab6\untitled\CherkesLab6.txt)");
...
void MainWindow::task3() {
  dialog = new QDialog(this);
  dialog->setWindowTitle(tr("Вариант 18 – double, разделитель '='"));
...
void MainWindow::saveArrayToFile() {
  ...
  QFile file(
      "C:\\Users\\Pojarnik\\Home\\MyQtLab\\Lab6\\untitled\\CherkesLab6_0.txt");
  if (file.open(QIODevice::WriteOnly | QIODevice::Text)) {
...
void MainWindow::task5() {
  ...
  connect(
      ...
      [this, &shoe, amountLineEdit, priceLineEdit, genderLineEdit,
       inStockLineEdit]() {
        bool ok;
        int amount = amountLineEdit->text().toInt(&ok);
        if (!ok) {
          QMessageBox::critical(this, "Ошибка",
                                "Некорректное значение в поле 'Количество'.");
          return;
        }
        shoe.amount = amount;
        ...
        if (!ok) {
          QMessageBox::critical(this, "Ошибка",
                                "Некорректное значение в поле 'Цена'.");
          return;
        }
        shoe.price = price;
        ...
        if (genderText == "f") {
          ...
          QMessageBox::critical(this, "Ошибка",
                                "Некорректное значение в поле 'Пол'.");
          return;
        }
        ...
        if (inStockText == "yes") {
          ...
          QMessageBox::critical(this, "Ошибка",
                                "Некорректное значение в поле 'В наличии'.");
          return;
        }

        QString filename =
            "C:\\Users\\Pojarnik\\Home\\MyQtLab\\Lab6\\untitled\\CherkesLab6_1."
            "txt";
        ...
        if (file.is_open()) {
          ...
          QMessageBox::information(this, "Сохранено",
                                   "Данные о обуви сохранены в файл.");
          dialog->accept();
        } else {
...
void MainWindow::task6() {
  ...
  QString filename =
      "C:\\Users\\Pojarnik\\Home\\MyQtLab\\Lab6\\untitled\\CherkesLab6_1.bin";
  ...
  connect(
      ...
      [this, &shoe, amountLineEdit, priceLineEdit, genderLineEdit,
       inStockLineEdit]() {
        ...
        QString filename =
            "C:\\Users\\Pojarnik\\Home\\MyQtLab\\Lab6\\untitled\\CherkesLab6_2."
            "bin";
        std::ofstream file(filename.toStdString(),
                           std::ios::binary | std::ios::app);

        if (file.is_open()) {
          file.write(reinterpret_cast<char*>(&shoe.amount),
                     sizeof(shoe.amount));
          file.write(reinterpret_cast<char*>(&shoe.price), sizeof(shoe.price));
          file.write(reinterpret_cast<char*>(&shoe.gender),
                     sizeof(shoe.gender));
          file.write(reinterpret_cast<char*>(&shoe.inStock),
                     sizeof(shoe.inStock));
          file.write(Shoes::getMaterialsString().toStdString().c_str(),
                     Shoes::getMaterialsString().length());
          file.write(Shoes::getSizesString().toStdString().c_str(),
                     Shoes::getSizesString().length());
          file.close();
          QMessageBox::information(this, "Сохранено",
                                   "Данные о обуви сохранены в файл.");
          dialog->accept();
...
void MainWindow::task8() {
  ...
  QString filename =
      "C:\\Users\\Pojarnik\\Home\\MyQtLab\\Lab6\\untitled\\CherkesLab6_2.bin";
  std::ifstream file(filename.toStdString(), std::ios::binary);

  if (file.is_open()) {
    Shoes shoe;
    while (
        file.read(reinterpret_cast<char*>(&shoe.amount), sizeof(shoe.amount)) &&
        ...
        file.read(reinterpret_cast<char*>(&shoe.inStock),
                  sizeof(shoe.inStock))) {
...
void MainWindow::task9() {
  ...
  connect(lineEdit, &QLineEdit::returnPressed, [this, lineEdit]() {
    ...
    QString filename =
        "C:\\Users\\Pojarnik\\Home\\MyQtLab\\Lab6\\untitled\\CherkesLab6_3.txt";
    QFile file(filename);
