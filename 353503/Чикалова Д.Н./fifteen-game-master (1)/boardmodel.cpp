...
BoardModel::BoardModel()
    : _board(_get_solved_board()),// двумерный массив хранящий состояние доски
      _nul_index(GAME_SIZE - 1),
      _is_solved(true),// флаг проверяющий решена ли растановка
      _start_board(_board), //сохраняет начальное состояние доски
      _start_nul_index(_nul_index),//сохраняет начальное состояние доски
      _start_is_solved(_is_solved), //сохраняет начальное состояние доски
      _num_shifts(0),// количество перемещений
      _states{} {}  //список который хранит предыдущее состояние доски
...
BoardModel::BoardModel(const bool is_rnd, const int complexity)
    ...
    sound.setSource(QUrl::fromUserInput("qrc:/zvuk-nopki-v-kompyuternoy-igre1.wav"));
}
