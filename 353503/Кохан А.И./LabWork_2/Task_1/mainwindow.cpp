    ...
    , ui(new Ui::MainWindow)
{
    ...
    txtFile = new QFile("/home/artem_kokhan/QT/2lab/Dates.txt");
    ...
    //checkT = new QTimer(this);
    connect(timer, &QTimer::timeout, this, &MainWindow::calendarSlot);
    connect(timer, &QTimer::timeout, this, &MainWindow::calendarSlot);
    //connect(checkT, &QTimer::timeout, this, &MainWindow::check);
    timer->start(SPEED_TIMER);
    //checkT->start(SPEED_TIMER);
}
...
{
    if (ui->centralwidget->isActiveWindow()) {
        txtFile->open(QIODevice::ReadOnly | QIODevice::Text);
        ui->tableWidget->setRowCount(0);
        QTextStream in(txtFile);
        QStringList lineWords;
