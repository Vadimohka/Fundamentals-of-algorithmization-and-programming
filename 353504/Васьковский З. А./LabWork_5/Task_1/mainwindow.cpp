    ...
    , ui(new Ui::MainWindow)
{
    ...
    ui->startButton->setStyleSheet("QPushButton {"
                                   "  background-color: #008631;" // Changed to green
                                   "}"
                                   "QPushButton:disabled {"
                                   "  background-color: #AAAAAA;" // Changed from #CCCCCC to #AAAAAA
                                   "  color: #444444;" // Changed from #666666 to #444444
                                   "}");

    ui->stopButton->setStyleSheet("QPushButton {"
                                  "  background-color: #FF5733;" // Changed from #008631 to #FF5733
                                  "}"
                                  "QPushButton:disabled {"
                                  "  background-color: #AAAAAA;" // Changed from #CCCCCC to #AAAAAA
                                  "  color: #444444;" // Changed from #666666 to #444444
    ...
    ui->languageChangeComboBox->setStyleSheet("QComboBox {"
                                              "  background-color: #0000FF;" // Changed from #008631 to #0000FF (blue)
                                              "}"
                                              "QComboBox:disabled {"
                                              "  background-color: #AAAAAA;" // Changed from #CCCCCC to #AAAAAA
                                              "  color: #444444;" // Changed from #666666 to #444444
...
void MainWindow::on_languageChangeComboBox_currentIndexChanged(int index)
{
    ...
    currTest.clear(); // Очистка currTest с помощью метода clear()
...
{
    QString fileName = QFileDialog::getOpenFileName(this, tr("Открытие"), "/home/zahar/Рабочий стол/Task_1", tr("Текстовые документы (*.txt)"));
...
void MainWindow::workWithEvent(QKeyEvent *event)
{
    ...
    if (currNeededSimb == currTest[currTestNum].size())
    {
        currNeededSimb = 0;
        currTestNum++;
        if (currTestNum == currTest.size() - 2)
        {
