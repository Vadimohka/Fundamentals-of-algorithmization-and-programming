...
MyStdString::MyStdString(const MyStdString& other) : size_(other.size_) {
    data_ = std::unique_ptr<char[]>(new char[size_ + 1]);
    std::memcpy(data_.get(), other.data_.get(), size_ + 1);
    reserved_ = size_ + 1;
}

MyStdString::MyStdString(size_t reserve) : data_(std::unique_ptr<char[]>(new char[reserve + 1])), reserved_(reserve + 1), size_(0) {
    data_[0] = '\0';
}
